...
var Phone = _sparkCore.SparkPlugin.extend({
  ...
  register: function register() {
    ...
    return this.spark.internal.device.refresh().then(function () {
      ...
      return _promise2.default.all([_this.spark.internal.mercury.when('event:mercury.buffer_state').then(function (_ref) {
        var _ref2 = (0, _slicedToArray3.default)(_ref, 1),
            message = _ref2[0];

        if (message.data.bufferState.locus === 'UNKNOWN') {
          return _this.spark.internal.locus.list();
        }
        return _promise2.default.resolve();
      }).then(function (loci) {
        if (!loci) {
  ...
  initialize: function initialize() {
    ...
    this.listenTo(this.spark.internal.mercury, 'change:connected', function () {
      _this3.connected = _this3.spark.internal.mercury.connected;
      _this3.registered = !!_this3.spark.internal.device.url && _this3.connected;
    });
  ...
  _onLocusEvent: function _onLocusEvent(event) {
    if ((0, _stateParsers.shouldRing)(event)) {
